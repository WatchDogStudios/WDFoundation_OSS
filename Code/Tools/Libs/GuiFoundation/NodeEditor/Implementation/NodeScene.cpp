...
#include <GuiFoundation/NodeEditor/CommentNode.h>
...
nsRttiMappedObjectFactory<nsQtCommentNode> nsQtNodeScene::s_CommentFactory;
...
void nsQtNodeScene::InitScene(const nsDocumentNodeManager* pManager)
{
  ...
  for (const auto& pObject : rootObjects)
  {
    /// NOTE: Cant do this, nsQtCommentNode is not a reflected type.
    /*
    if (pObject->GetType()->IsDerivedFrom<nsQtCommentNode>())
    {
      CreateQtComment(pObject);
    }
    */
  }
}
...
void nsQtNodeScene::keyPressEvent(QKeyEvent* event)
{
  ...
  {
    /// TODO: Finsh Node Comments.
    CreateQtComment(nullptr);
  }
}
...
void nsQtNodeScene::CreateQtNode(const nsDocumentObject* pObject)
{
  ...
  // Note: We dont create connections here as it can cause recursion issues
  if (m_pTempConnection)
  {
    m_pTempNode = pNode;
  }
...
void nsQtNodeScene::CreateQtComment(const nsDocumentObject* pObject)
{
}

void nsQtNodeScene::DeleteQtComment(const nsDocumentObject* pObject)
{
}
...
void nsQtNodeScene::CreateCommentGroupAction(const nsDocumentObject* nodes, const nsVec2& vPos, const nsVec2& vSize)
{
  nsCommandHistory* history = GetDocumentNodeManager()->GetDocument()->GetCommandHistory();
  history->StartTransaction("Add Comment Group");
  ...
  {
    nsAddObjectCommand cmd;
    cmd.m_pType = nodes->GetTypeAccessor().GetType();
    cmd.m_NewObjectGuid = nsUuid::MakeUuid();
    cmd.m_Index = -1;
    ...
    {
      nsSetObjectPropertyCommand setCmd;
      setCmd.m_Object = pNode->GetObject()->GetGuid();
      setCmd.m_sProperty = "CommentGroup";
      setCmd.m_NewValue = cmd.m_NewObjectGuid;
      res = history->AddCommand(setCmd);
      if (res.m_Result.Failed())
        break;
    }
