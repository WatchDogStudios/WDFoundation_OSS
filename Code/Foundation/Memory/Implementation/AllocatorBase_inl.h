...
NS_ALWAYS_INLINE nsAllocatorBase::nsAllocatorBase() = default;
...
namespace nsInternal
{
  ...
  struct NewInstance
  {
    NS_ALWAYS_INLINE NewInstance(T* pInstance, nsAllocatorBase* pAllocator)
    {
  ...
  template <typename T>
  NS_FORCE_INLINE void Delete(nsAllocatorBase* pAllocator, T* pPtr)
  {
    if (pPtr != nullptr)
    {
  ...
  template <typename T>
  NS_FORCE_INLINE T* CreateRawBuffer(nsAllocatorBase* pAllocator, size_t uiCount)
  {
  ...
  NS_FORCE_INLINE void DeleteRawBuffer(nsAllocatorBase* pAllocator, void* pPtr)
  {
    if (pPtr != nullptr)
    {
      pAllocator->Deallocate(pPtr);
  ...
  template <typename T>
  inline nsArrayPtr<T> CreateArray(nsAllocatorBase* pAllocator, nsUInt32 uiCount)
  {
    T* buffer = CreateRawBuffer<T>(pAllocator, uiCount);
    nsMemoryUtils::Construct(buffer, uiCount);

    return nsArrayPtr<T>(buffer, uiCount);
  }

  template <typename T>
  inline void DeleteArray(nsAllocatorBase* pAllocator, nsArrayPtr<T> arrayPtr)
  {
  ...
  template <typename T>
  NS_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, nsAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, nsTypeIsPod)
  {
    return (T*)pAllocator->Reallocate(pPtr, uiCurrentCount * sizeof(T), uiNewCount * sizeof(T), NS_ALIGNMENT_OF(T));
  }

  template <typename T>
  NS_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, nsAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, nsTypeIsMemRelocatable)
  {
    return (T*)pAllocator->Reallocate(pPtr, uiCurrentCount * sizeof(T), uiNewCount * sizeof(T), NS_ALIGNMENT_OF(T));
  }

  template <typename T>
  NS_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, nsAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount, nsTypeIsClass)
  {
  ...
  template <typename T>
  NS_FORCE_INLINE T* ExtendRawBuffer(T* pPtr, nsAllocatorBase* pAllocator, size_t uiCurrentCount, size_t uiNewCount)
  {
