template <nsAllocatorTrackingMode TrackingMode>
nsLinearAllocator<TrackingMode>::nsLinearAllocator(nsStringView sName, nsAllocator* pParent)
  : nsAllocatorWithPolicy<nsAllocPolicyStack, TrackingMode>(sName, pParent)
...
template <nsAllocatorTrackingMode TrackingMode>
nsLinearAllocator<TrackingMode>::~nsLinearAllocator()
{
  Reset();
}

template <nsAllocatorTrackingMode TrackingMode>
void* nsLinearAllocator<TrackingMode>::Allocate(size_t uiSize, size_t uiAlign, nsMemoryUtils::DestructorFunction destructorFunc)
{
  NS_LOCK(m_Mutex);

  void* ptr = nsAllocatorWithPolicy<nsAllocPolicyStack, TrackingMode>::Allocate(uiSize, uiAlign, destructorFunc);
...
template <nsAllocatorTrackingMode TrackingMode>
void nsLinearAllocator<TrackingMode>::Deallocate(void* pPtr)
{
  NS_LOCK(m_Mutex);
  ...
  nsAllocatorWithPolicy<nsAllocPolicyStack, TrackingMode>::Deallocate(pPtr);
}
...
template <nsAllocatorTrackingMode TrackingMode>
void nsLinearAllocator<TrackingMode>::Reset()
{
  NS_LOCK(m_Mutex);
