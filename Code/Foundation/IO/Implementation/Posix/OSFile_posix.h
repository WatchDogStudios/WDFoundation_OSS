...
{
  FILE* pFile = fopen(nsString(sFile), "r");
...
#  if NS_ENABLED(NS_PLATFORM_OSX)
    ...
    if (bundlePath != nullptr)
    {
      ...
      nsArrayPtr<char> temp = NS_DEFAULT_NEW_ARRAY(char, maxSize);
...
#  else
    ...
    nsStringBuilder path(nsStringView(result, result + length));
