    ...
    if (!pendingRemoveOrRename.IsEmpty())
    {
      if (pendingRemoveOrRename.isDirectory)
      {
        if (whatToWatch.IsSet(Watch::Deletes))
        {
          ...
          {
            mirror->Enumerate(pendingRemoveOrRename.path, [&](const nsStringBuilder& sPath, nsFileSystemMirrorType::Type type) { func(sPath, nsDirectoryWatcherAction::Removed, (type == nsFileSystemMirrorType::Type::File) ? nsDirectoryWatcherType::File : nsDirectoryWatcherType::Directory); })
              .AssertSuccess();
          }
