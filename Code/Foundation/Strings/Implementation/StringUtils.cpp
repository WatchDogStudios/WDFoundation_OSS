...
nsAtomicInteger32 nsStringUtils::g_MaxUsedStringLength;
nsAtomicInteger32 nsStringUtils::g_UsedStringLengths[256];

void nsStringUtils::AddUsedStringLength(nsUInt32 uiLength)
{
...
void nsStringUtils::PrintStringLengthStatistics()
{
  NS_LOG_BLOCK("String Length Statistics");

  nsLog::Info("Max String Length: {0}", (nsInt32)g_MaxUsedStringLength);

  nsUInt32 uiCopiedStrings = 0;
  for (nsUInt32 i = 0; i < 256; ++i)
    uiCopiedStrings += g_UsedStringLengths[i];

  nsLog::Info("Number of String Copies: {0}", uiCopiedStrings);
  nsLog::Info("");

  nsUInt32 uiPercent = 0;
  nsUInt32 uiStrings = 0;
  for (nsUInt32 i = 0; i < 256; ++i)
  {
    if (100.0f * (uiStrings + g_UsedStringLengths[i]) / (float)uiCopiedStrings >= uiPercent)
    {
      nsLog::Info("{0}%% of all Strings are shorter than {1} Elements.", nsArgI(uiPercent, 3), nsArgI(i + 1, 3));
      uiPercent += 10;
    }
...
nsUInt32 nsStringUtils::Copy(char* szDest, nsUInt32 uiDstSize, const char* szSource, const char* pSourceEnd)
{
  ...
  {
    nsStringUtils::AddUsedStringLength(0);
    szDest[0] = '\0';
    return 0;
  }
...
// Check if we just copied half a UTF-8 character
  ...
  if (uiBytesToCopy > 0)
  {
    ...
    ptrdiff_t expectedLength = nsUnicodeUtils::GetUtf8SequenceLength(*szUtf8StartByte);
  ...
  *szLastCharacterPos = '\0';
  ...
  const nsUInt32 uiLength = (nsUInt32)(szLastCharacterPos - szDest);

  nsStringUtils::AddUsedStringLength(uiLength);
  return uiLength;
}
...
nsUInt32 nsStringUtils::CopyN(char* szDest, nsUInt32 uiDstSize, const char* szSource, nsUInt32 uiCharsToCopy, const char* pSourceEnd)
{
  ...
  *szLastCharacterPos = '\0';
  ...
  const nsUInt32 uiLength = (nsUInt32)(szLastCharacterPos - szStartPos);
...
bool nsStringUtils::StartsWith_NoCase(const char* szString, const char* szStartsWith, const char* pStringEnd, const char* szStartsWithEnd)
{
  ...
  while ((*szString != '\0') && (szString < pStringEnd))
  {
    ...
    nsUnicodeUtils::MoveToNextUtf8(szString, pStringEnd);
    nsUnicodeUtils::MoveToNextUtf8(szStartsWith, szStartsWithEnd);
  }
...
bool nsStringUtils::EndsWith_NoCase(const char* szString, const char* szEndsWith, const char* pStringEnd, const char* szEndsWithEnd)
{
  ...
  while (pCur1 > szString)
  {
    ...
    nsUnicodeUtils::MoveToPriorUtf8(pCur1);
    nsUnicodeUtils::MoveToPriorUtf8(pCur2);
...
const char* nsStringUtils::FindSubString(const char* szSource, const char* szStringToFind, const char* pSourceEnd, const char* szStringToFindEnd)
{
  ...
  while ((pCurPos < pSourceEnd) && (*pCurPos != '\0'))
  {
    ...
    nsUnicodeUtils::MoveToNextUtf8(pCurPos, pSourceEnd);
  }
...
const char* nsStringUtils::FindWholeWord(const char* szString, const char* szSearchFor, NS_CHARACTER_FILTER isDelimiterCB, const char* pStringEnd)
{
  ...
  while ((*pCurPos != '\0') && (pCurPos < pStringEnd))
  {
    ...
    pPrevPos = pCurPos;
    nsUnicodeUtils::MoveToNextUtf8(pCurPos, pStringEnd);
  }

  return nullptr;
}

const char* nsStringUtils::FindWholeWord_NoCase(
  const char* szString, const char* szSearchFor, NS_CHARACTER_FILTER isDelimiterCB, const char* pStringEnd)
{
  // Handle nullptr-pointer strings
  if ((IsNullOrEmpty(szString)) || (IsNullOrEmpty(szSearchFor)))
    return nullptr;
...
const char* nsStringUtils::SkipCharacters(const char* szString, NS_CHARACTER_FILTER skipCharacterCB, bool bAlwaysSkipFirst)
{
  ...
  while (*szString != '\0')
  {
    ...
    nsUnicodeUtils::MoveToNextUtf8(szString);
  }
...
NS_STATICLINK_FILE(Foundation, Foundation_Strings_Implementation_StringUtils);
