...
nsStatus nsMiniDumpUtils::WriteProcessMiniDump(nsStringView sDumpFile, nsUInt32 uiProcessID, nsMinWindows::HANDLE hProcess, struct _EXCEPTION_POINTERS* pExceptionInfo)
{
  ...
  if (opt_FullCrashDumps.GetOptionValue(nsCommandLineOption::LogMode::Always))
  {
...
nsStatus nsMiniDumpUtils::WriteOwnProcessMiniDump(nsStringView sDumpFile, struct _EXCEPTION_POINTERS* pExceptionInfo)
{
  return WriteProcessMiniDump(sDumpFile, GetCurrentProcessId(), GetCurrentProcess(), pExceptionInfo);
}

nsStatus nsMiniDumpUtils::WriteExternalProcessMiniDump(nsStringView sDumpFile, nsUInt32 uiProcessID, nsMinWindows::HANDLE hProcess)
{
