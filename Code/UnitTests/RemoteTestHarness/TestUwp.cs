...
namespace nsUwpTestHarness
{
  ...
  class nsTestUWP
  {
    ...
    public nsTestUWP(string nsWorkspace, string testOutputPath,
    ...
    private string GetFileserverPath()
    {
      ...
      string absFSPath = System.IO.Path.Combine(absBinDir, "nsFileserve.exe");
    ...
    private void DeployAppX(out string fullPackageName)
    {
      ...
      string absSlnPath = Path.Combine(_nsWorkspace, "nsVs2022.sln");
      ...
      // "C:\Program Files (x86)\Microsoft Visual Studio 14.0\Common7\IDE\devenv.com" "F:\Development\current_development\nsEngine\build_uwp64\nsEngine.sln" /Deploy "Dev|x64" /project CoreTest
      var deployProcessResults = nsProcessHelper.RunExternalExe(devEnvPath,
    ...
    private void StartAppX(string fullPackageName, string args, out uint pid)
    {
      ...
      //
      // The following method seems to work just fine for nsEngine projects if not for all AppX in general.
      //
      ...
      string appUserModelId = appFamilyName + "!App"; // Apply our knowledge of nsEngine's appx manifests.
    ...
    public void ExecuteTest()
    {
      ...
      Func<nsProcessHelper.ProcessResult> startFileserve = () =>
      {
        Console.WriteLine("Starting Fileserve ...");

        // 60s timeout for connect, 2s timeout for closing after connection loss.
        string args = string.Format("-specialdirs nstest \"{0}\" -fs_start -fs_wait_timeout 120 -fs_close_timeout 4", _absTestOutputDirectory);
        return nsProcessHelper.RunExternalExe(absFilerserveFilename, args, absBinDir, fileserveTimeoutMS);
      };

      // create a real time user mode session
      using (var session = new TraceEventSession("nsETWMonitorSession"))
      {
 
        session.EnableProvider(new Guid("BFD4350A-BA77-463D-B4BE-E30374E42494")); //nsLogProvider
        session.Source.Dynamic.AddCallbackForProviderEvent("nsLogProvider", "LogMessage", delegate (TraceEvent data)
        {
          ...
          if (Type != (int)nsLogMsgType.EndGroup)
          {
            Console.Out.WriteLine("".PadLeft(Indentation) + Text);
          }

          if (Type == (int)nsLogMsgType.InfoMsg && Text == "All tests passed.")
          {
            Interlocked.Exchange(ref this._failed, 0);
            Interlocked.Exchange(ref this._finished, 1);
          }
          else if (Type == (int)nsLogMsgType.InfoMsg && Text != null && Text.StartsWith("Tests failed: ") && Text.Contains("Tests passed: "))
          {
        ...
        using (Task<nsProcessHelper.ProcessResult> runFileServe = Task.Factory.StartNew(startFileserve, System.Threading.Tasks.TaskCreationOptions.LongRunning))
        {
          ...
          if (!appXProcess.WaitForExit(5000))
          {
            ...
            try
            {
              ...
              nsProcessHelper.RunExternalExe("procdump", args, absBinDir);
            }
            catch (Exception e)
            {
